...
class MagicPhrasesFileService:
    ...
    def read_phrases():
        ...
        magic_phrases_list = []
        # try:
        f = open("magic_phrases.txt", "r")
        for line in f.readlines():
            split_line = line.split(".")
            phrase = Phrase.Phrase()
            word_list = split_line[0].split(" ")
            for index in range(len(word_list)):
                if index == 0:
                    phrase.add_word(Word.Word(word_list[index].lower(), WordType.WordType.UNKNOWN))
                else:
                    phrase.add_word(Word.Word(word_list[index], WordType.WordType.UNKNOWN))
            magic_phrases_list.append(phrase)
        f.close()
        # except Exception:
        #     print("Could not read file")
        return magic_phrases_list
